...
double timeToTemperature(int t){
	return (100-(0.1*pow((double)t,2))); /* formula containing t: linear dicreas */
}
...
int likelihood(int val, double temp){
	srand((unsigned int)time(NULL));
	double x = rand() % 1, y = exp(val/temp);
	
	return (x < y ? TRUE : FALSE);
}

/* The idea: Stochastic Hill climbing compined with temperature-function. 
 * We always accept the first better state (than the current state), but
 * otherwise we accept a worse state with a certain likelihood. 
 * Likelihood : accept state if e^(val/temp) > random value.
 */
 
void simulatedAnnealing() {
	int optimum = (nqueens-1)*nqueens/2, best = evaluateState();
	int t = 1, x, val = 0, queen, column;
	double temp=3;
	//printf("best: %d, optimum: %d\n", best, optimum);
	while (best != optimum && temp > 0 ){
		...
		for (queen=0; queen < nqueens; queen++){
			...
			for (column=0; column < nqueens; column++){
				neighbor[queen] = column;
				val = evaluateStateHill(2)-evaluateState();
				//printf("val: %d, temp: %lf\n", val, temp);
				if (val > 0){
					best = val;
					for (x=0; x < nqueens; x++){
						queens[x] = neighbor[x];
	...
	}; /* runs until solution (global maximum) is found or T (temp) == 0 */
	printf("Final evaluation=%d\n", evaluateStateHill(1));
	printf ("Final state is");
	printState();
}
